...
class Signal_process:
    ...
    def level_crossings(
        self, num_levels=500, reference_level_type="mean", gate=0, count_on_level=False
    ):

        reference_level = (
            np.mean(self.signal_amp) if reference_level_type == "mean" else 0
        )
        levels = np.linspace(
            np.min(self.signal_amp), np.max(self.signal_amp), num_levels
        )
        ...
        for i in range(1, len(self.signal_amp)):
            for level in levels:
                if (self.signal_amp[i - 1] < level and self.signal_amp[i] >= level) or (
                    self.signal_amp[i - 1] > level and self.signal_amp[i] <= level
                ):
                    crossings[level] += 1

        test_crossing_counts = {level: count for level, count in crossings.items()}
        test_crossing_counts_df = pd.DataFrame(
            list(test_crossing_counts.items()), columns=["Level", "CrossingCount"]
        )
        cross_x, level_y = (
            test_crossing_counts_df["CrossingCount"],
            test_crossing_counts_df["Level"],
        )
        ...
        for g in dict_data:
            for dirs in dict_data.get(g):
                ...
                sorted_keys = sorted(
                    a_dict.keys(),
                    key=lambda x: (
                        event_list.index(x) if x in event_list else float("inf")
                    ),
                )
...
def split_dict(input_dict, chunk_size):
    ...
    return [
        {key: input_dict[key] for key in keys[i : i + chunk_size]}
...
if __name__ == "__main__":
    ...
    r_f = R_file(
        "/nobackup/vol01/a420192/GSO/CNA_10476_V1_Bolt_grille/cvm_accel/it3/Loaded/"
    )
    ...
    map_dict = {
        "20720001": [
            "16_HoodCrossbar_Psg_Ax",
            "17_HoodCrossbar_Psg_Ay",
            "18_HoodCrossbar_Psg_Az",
        ],
        "20720002": [
            "13_HoodCrossbar_Drv_Ax",
            "14_HoodCrossbar_Drv_Ay",
            "15_HoodCrossbar_Drv_Az",
        ],
        "20720003": [
            "7_Grille_Drv_UpperMount_Ax",
            "8_Grille_Drv_UpperMount_Ay",
            "9_Grille_Drv_UpperMount_Az",
        ],
        "20720004": [
            "10_Grille_Psg_UpperMount_Ax",
            "11_Grille_Psg_UpperMount_Ay",
            "12_Grille_Psg_UpperMount_Az",
        ],
        "20720005": [
            "1_Grille_Drv_LowerMount_Ax",
            "2_Grille_Drv_LowerMount_Ay",
            "3_Grille_Drv_LowerMount_Az",
        ],
        "20720006": [
            "4_Grille_Psg_LowerMount_Ax",
            "5_Grille_Psg_LowerMount_Ay",
            "6_Grille_Psg_LowerMount_Az",
        ],
    }
    ...
    seq_dict = r_f.event_seq_creator(
        g_dict, ["WEST-STRAIGHT", "CORNER-BUMPS", "NORTH-STRAIGHT", "EAST-STRAIGHT"]
    )

    fig, axes = plt.subplots(5, 5, figsize=(15, 10), sharey="row")
    cnt_x = 0
    cnt_y = 0
    for num, ev in enumerate(
    ...
    seq_crossings, seq_levels = seq_signal_obj.level_crossings(
        num_levels, reference_level_type, gate, count_on_level
    )
    se_freq, seq_psd = seq_signal_obj.psd()

    # Test data LCR
    signal_object_test = Signal_process(x1_reorder, y1)
    test_crossings, test_levels = signal_object_test.level_crossings(
        num_levels, reference_level_type, gate, count_on_level
    )
    test_freq, test_psd = signal_object_test.psd()

    level_cross_fig = fig.add_subplot(
        5, 1, 4
    )  # grid arrangment 3 row 1 column position 3
