...
class ReadSignalFrame(ttk.Frame):
    ...
    def ui(self):
        ...
        self.button = tk.Button(
            self.top_frame, text="Select R files Folder", command=self.select_folder
        )
        ...
        self.button_csv = tk.Button(
            self.top_frame_csv, text="Select csv file(s)", command=self.select_csv_file
        )
        ...
        label_title_r = tk.Label(
            self.left_frame, text="CVM/Simulation DATA", font=("Arial", 14, "bold")
        )
        ...
        self.listbox_event = tk.Listbox(
            self.left_frame, exportselection=False, selectmode=tk.MULTIPLE
        )
        ...
        label_title_csv = tk.Label(
            self.right_frame_csv, text="CSV/Test DATA", font=("Arial", 14, "bold")
        )
        self.label_csv_ok = tk.Label(self.right_frame_csv, text="CSV File : None")

        self.listbox_csv = tk.Listbox(
            self.right_frame_csv,
            exportselection=False,
            selectmode=tk.MULTIPLE,
            width=50,
        )
        self.listbox_csv_file = tk.Listbox(
            self.right_frame_csv, exportselection=False, selectmode=tk.SINGLE, width=50
        )
...
class TestVsSim(ttk.Frame):
    ...
    def update_plot(self):
        ...
        seq_t, seq_amp = self.event_org.event_seq_creator_on_select(
            event_dict,
            self.shared_data.selected_sim_comb,
            grid_id,
            grid_dir,
            self.shared_data.sim_combine,
        )
